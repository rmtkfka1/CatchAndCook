...
// 캡슐 간 충돌 검사 함수
bool BoundingCapsule::Intersects(const BoundingCapsule& capA, const BoundingCapsule& capB)
{
    // 캡슐의 중심 선분을 DirectXMath 벡터로 로드
    XMVECTOR A0 = XMLoadFloat3(&capA.p0);
    XMVECTOR A1 = XMLoadFloat3(&capA.p1);
...
bool BoundingCapsule::Intersects(const BoundingCapsule& capA,const BoundingBox& box)
{
    ...
    float sqDist = DistanceSegmentAABBSquared(capA.p0,capA.p1,boxMin,boxMax);
    return sqDist <= capA.radius * capA.radius;
}

// (D) 캡슐–OBB 충돌 판정
bool BoundingCapsule::Intersects(const BoundingCapsule& capA,const BoundingOrientedBox& obb)
{
    ...
    XMVECTOR p0World = XMLoadFloat3(&capA.p0);
    XMVECTOR p1World = XMLoadFloat3(&capA.p1);
...
// (E) 캡슐–프러스텀 충돌 판정
bool BoundingCapsule::Intersects(const BoundingCapsule& capA, const BoundingFrustum& frustum)
{
    XMVECTOR p0Vec = XMLoadFloat3(&capA.p0);
    XMVECTOR p1Vec = XMLoadFloat3(&capA.p1);
...
bool BoundingCapsule::Intersects(const BoundingCapsule& capA, FXMVECTOR rayOrigin, FXMVECTOR rayDir, float maxDistance,
	float* outDistance)
{
    XMVECTOR A = XMLoadFloat3(&capA.p0);
    XMVECTOR B = XMLoadFloat3(&capA.p1);
    float r = capA.radius;
