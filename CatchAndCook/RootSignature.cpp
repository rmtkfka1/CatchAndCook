...
void RootSignature::InitGraphicsRootSignature()
{
	...
	param[GLOBAL_SRV_MAO_INDEX].InitAsDescriptorTable(1, &G_MAO_SRV_RANGE);
	...
	{
		srv_str_ranges[i] = CD3DX12_DESCRIPTOR_RANGE(
			D3D12_DESCRIPTOR_RANGE_TYPE_SRV,1,SRV_STRUCTURED_TABLE_REGISTER_OFFSET + i);
		param[SRV_STRUCTURED_TABLE_INDEX + i].InitAsDescriptorTable(1,&srv_str_ranges[i]);
	}
